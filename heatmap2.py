"""
From: http://code.activestate.com/recipes/578834-hierarchical-clustering-heatmap-python/
...
class HiearchicalHeatmap(object):
    ...
    color_gradient = 'jet'   # Can be: see color_gradients dictionary
    fig_weight = 7
    fig_height = 12
    ...
    def plot(self):
        ...
        norm = matplotlib.colors.LogNorm(value_min, value_max)
        ...
        hmap = axm.matshow(self.frame, aspect='auto', origin='lower', cmap=cmap, norm=norm)
        axm.set_xticks([])
        axm.set_yticks([])
        ...
        for i in range(self.frame.shape[0]):
            axm.text(self.frame.shape[1]-0.5, i, '  ' + row_header[idx1[i]], verticalalignment="center", size='x-small')
            new_row_header.append(row_header[idx1[i]] if self.row_method else row_header[i])
        for i in range(self.frame.shape[1]):
            axm.text(i, 0, ' '+column_header[idx2[i]], rotation=45, size='x-small',
        ...
        cb = matplotlib.pyplot.colorbar(hmap, ax=axcb, aspect=3, fraction=0.9, shrink=1.1, orientation='horizontal')
        axcb.set_yticks([])
        axcb.set_xticks([])
        cb.set_label('RPKM', size='x-small')
        cb.ax.tick_params(labelsize='x-small')
        axcb.set_title("Colorkey", size='x-small')
        #max_cb_ticks = 5
        #axcb.xaxis.set_major_locator(pyplot.MaxNLocator(max_cb_ticks))

        # Render the graphic #
        #if len(row_header)>50 or len(column_header)>50: matplotlib.rcParams['font.size'] = 4
        #else: matplotlib.rcParams['font.size'] = 5
...
class MyHeatmap(HiearchicalHeatmap):
    short_name = 'My_Heatmap'

    def plot(self, df):
        self.frame = df
        self.path = '/tmp/' + self.short_name + '.png'
        fig, axm, axcb, cb = HiearchicalHeatmap.plot(self)
        print('Done running MyHeatmap')
...
if __name__ == '__main__':
    ...
    basedir = '/Users/saltikov/PycharmProjects/rna_seq/RUN1/'
    df1 = pandas.DataFrame.from_csv(basedir+'RUN1_data.csv', index_col=0)

    # columns with the specific RPKM values
    cols = [0, 1, 2, 9, 10, 11, 18, 19, 20]

    # Sorted list of DE genes
    df2 = pandas.DataFrame.from_csv('/Users/saltikov/PycharmProjects/rna_seq/RUN1/DESEQ/DESeq_RUN1_FCtable.csv',
                                index_col='Gene')
    print(df2.columns.tolist())

    gs = hmp.GeneScan()
    genes = gs.getgenes(df2, 'logFC_FumAs', 'p_FumAs', 3)
    dfmap = gs.filterrpkm(df1, genes, cols) +1
    ...
    title = dict(title='RUN1_DEHeatMaps', output=True)

    if title['output']:
        matplotlib.pyplot.savefig(basedir+title['title'] + '.png', format='png', dpi=200, bbox_inches='tight')
